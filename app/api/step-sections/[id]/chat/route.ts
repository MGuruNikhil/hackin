...
import { ideas, stepSectionChats, stepSections, stepTodos } from "@/lib/schema"
...
export async function POST(
	...
	try {
		...
		console.log(
			`[API] Processing chat for section ${sectionId}, user message: "${userMessage}"`,
		)
		...
		console.log("[API] Section details:", {
			id: section.id,
			title: section.title,
		})
...
Help the user with questions about this section, provide guidance, discuss implementation details, and offer suggestions for completing the tasks.`,
			...
			onFinish: async finishResult => {
				console.log(
					"[API] AI response finished - Text length:",
					finishResult.text?.length || 0,
				)
		...
		return new Response(`Failed to process chat message: ${error}`, {
			status: 500,
		})
	}
}
